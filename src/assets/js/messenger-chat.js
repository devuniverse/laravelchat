...
; (function () {
    ...
    function newMenu(messageClass, messageId) {
        ...
        return '\
            <i class="fa fa-ellipsis-h fa-2x '+ pull + '" aria-hidden="true">\
                <div class="delete" data-id="'+ messageId + '"><i class="fa fa-times"></i></div>\
            </i>\
        ';
    }
    ...
    function newMessage(message, messageClass, failed = 0) {
        ...
        $('.messenger-body').append('\
            <li class="clearfix animated '+ (message.whoseisit == 'mine' ? 'pulse' : 'zoom') + ' ' + message.whoseisit + '" id="' + message.convoid + '">\
    ...
    $(document).ready(function () {
         ...
         */
        $(document).on('mouseover', '.message-inner', function (e) {
            $(this).find('.message-action').show();
        });

        /**
         * Mouse up to remove menu dots.
         */
        $(document).on('mouseout', '.message-inner', function (e) {
            var deleteBtn = $(this).find('.delete').css('display');

            if (deleteBtn !== 'block') {
                $(this).find('.message-action').hide(); // only hide if delete popup is not poped up.
            }
        });
        ...
        $(document).on('click', '.message-action', function (e) {
            ...
            $('.message-action').not($(this).find('.message-action')).hide();
            // Only show this menu.
        ...
        $('body').on('click', function (e) {
            if ($(e.target).hasClass('message-action')) { // toggle delete menu on clicking on dots.
                return true;
            } else if ($(e.target).hasClass('message-inner')) { // don't hide on hover dots when click on message row.
                $('.message-action').not($(e.target).find('.message-action')).hide();
            } else {
                $('.message-action').hide();
            }
            $('.delete').hide();
